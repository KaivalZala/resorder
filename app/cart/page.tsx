...
export default function CartPage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-orange-50 to-red-50 px-4 sm:px-6 md:px-8 py-6">
      <div className="max-w-5xl mx-auto">
        {/* Header */}
        <div className="flex flex-col sm:flex-row sm:items-center gap-4 mb-6 animate-slide-up">
          <Button
            variant="ghost"
            size="icon"
            onClick={() => router.back()}
            className="hover:bg-white/50 backdrop-blur-sm rounded-full w-10 h-10"
          >
            <ArrowLeft className="h-5 w-5" />
          </Button>
          <div className="flex-1">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <Card className="bg-white/80 backdrop-blur-sm border-0 shadow-lg animate-slide-up">
              ...
              <CardContent>
                <Textarea
                  ...
                  value={specialNotes}
                  onChange={(e) => setSpecialNotes(e.target.value)}
                  className="resize-none bg-white/50 backdrop-blur-sm w-full"
                  rows={3}
                />
              </CardContent>
            </Card>
          ...
          {/* Bill Summary */}
          <div className="space-y-6">
            <Card className="bg-white/80 backdrop-blur-sm border-0 shadow-lg animate-slide-up sticky top-6">
              <CardHeader>
                <CardTitle className="flex items-center gap-2 text-base sm:text-lg">
                  <CreditCard className="h-5 w-5 text-green-500" />
                  Bill Summary
              ...
              <CardContent className="space-y-3 text-sm sm:text-base">
                ...
                <div className="flex justify-between text-xl font-bold bg-gradient-to-r from-green-600 to-emerald-600 bg-clip-text text-transparent">
                  <span>Total</span>
                  <span>₹{billing.total.toFixed(2)}</span>
                </div>
                ...
                <Button
                  onClick={() => router.push("/menu")}
                  className="w-full mt-2 bg-gradient-to-r from-orange-400 to-red-400 hover:from-orange-500 hover:to-red-500 text-white font-bold py-3 text-base sm:text-lg shadow-md hover:shadow-lg transition-all duration-300 hover:scale-105"
                >
                  <Plus className="h-5 w-5 mr-2" />
                  Add More Items
                </Button>
                ...
                <Button
                  onClick={() => setConfirmDialogOpen(true)}
                  disabled={placing || state.items.length === 0}
                  className="w-full mt-4 bg-gradient-to-r from-orange-500 to-red-500 hover:from-orange-600 hover:to-red-600 text-white font-bold py-4 text-base sm:text-lg shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105"
                >
                  {placing ? (
                    <>
                      <LoadingSpinner size="sm" className="mr-3" />

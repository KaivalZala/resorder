      ...
      <div className="max-w-6xl mx-auto p-8">
        <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
          {searchedItems.map((item, index) => (
            <Card
              key={item.id}
              className="overflow-hidden hover-lift bg-white/95 backdrop-blur-lg border border-[#C7C6A1] shadow-xl animate-slide-up rounded-3xl"
              style={{ animationDelay: `${index * 0.1}s` }}
            >
              <div className="relative">
                ...
                <div className="p-7">
                  <div className="flex justify-between items-start mb-4">
                    <h3 className="font-extrabold text-2xl text-[#7A7A4D] font-serif">{item.name}</h3>
                    <div className="flex items-center gap-1 text-yellow-500">
                      <Star className="h-5 w-5 fill-current" />
                      <span className="text-base font-semibold">4.8</span>
                    </div>
                  </div>
                  <p className="text-[#A6A67A] mb-5 line-clamp-2 font-medium">{item.description}</p>
                  <div className="flex justify-between items-center">
                    <div className="flex gap-2 flex-wrap">
                      {item.tags.map((tag) => (
                        <Badge
                          key={tag}
                          variant="secondary"
                          className="text-xs bg-gradient-to-r from-gray-100 to-gray-200 hover:from-gray-200 hover:to-gray-300 transition-all duration-300 rounded-full px-3 py-1 font-serif text-[#7A7A4D]"
                        >
                          {getTagIcon(tag)}
                    ...
                    <Button
                      ...
                      className="bg-gradient-to-r from-orange-500 to-red-500 hover:from-orange-600 hover:to-red-600 text-white shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105 rounded-full px-6 py-2 font-serif text-base"
                    >
